...
**Exercise:** Redo what we have done in boootstraping the mean of a sample from a normal distribution but instead boostrap the median and estimate the standard error and a 95% CI for the statistic. You can use the fact that the function `median` computes the sample median. How do the results compare and contrast with those of the mean?
