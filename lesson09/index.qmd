...
Whichever of the functions `prop.test` or `prop_test` we use to compute it, the p-value is about 0.69. We interpret this value as follows: 

> Assuming the null hypothesis to be true, there is about a 69% chance of observing a point estimate that is as or more extreme than the one we obtained from the data. Thus, the results of our coin toss are not at all rare so we should probably not consider our data as providing strong evidence against the null hypothesis. One would typically say, "we decide to fail to reject the null hypothesis." 
