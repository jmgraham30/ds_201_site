...
The R package `dplyr` which belongs to the `tidyverse` family of packages is one of the available implementations of the common data manipulation methods [@dplyr]. The `dplyr` package is robust, well-documented, easy-to-use, and efficient so we will explore it in this lesson. It is worth it to take a moment to visit the ['dplyr' website](https://dplyr.tidyverse.org/) as it contains helpful information and resources, [view the webpage](https://dplyr.tidyverse.org/). 
